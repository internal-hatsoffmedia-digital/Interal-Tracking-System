...
function ProjectDetail({ project, people, onClose, onSaved, onEdit }: { project: ManagedProject; people: ProjectPerson[]; onClose: () => void; onSaved: () => Promise<void>; onEdit?: () => void }) {
  ...
  return <WorkspaceDialog titleId="project-detail-title" onClose={()=>{if(!saving)onClose();}}>
    ...
    {manager ? <><div className="grid gap-4 sm:grid-cols-2">{(['assignee','shared'] as const).map(kind => <fieldset key={kind} className="coordinator-picker"><legend className="px-1 text-sm font-semibold">{kind === 'assignee' ? 'Current assignees' : 'Shared with'}</legend>{people.filter(u=>(u.is_active && u.role==='project_coordinator' && !!project.team_id && u.team_id===project.team_id) || project.project_members.some(m=>m.profile_id===u.id)).map(u => {
      const values = kind==='assignee' ? assignees : shared; const set = kind==='assignee' ? setAssignees : setShared;
      return <label key={u.id} className="coordinator-option"><input type="checkbox" checked={values.includes(u.id)} disabled={saving || ((!u.is_active || u.role!=='project_coordinator' || u.team_id!==project.team_id) && !values.includes(u.id))} onChange={e=>set(e.target.checked ? [...values,u.id] : values.filter(id=>id!==u.id))} /><span className="coordinator-avatar" aria-hidden="true">{(u.full_name ?? "?").slice(0,1)}</span><span>{u.full_name ?? u.id}<span className="block text-xs text-slate-500">{label(u.role)}{(!u.is_active || u.role!=='project_coordinator' || u.team_id!==project.team_id) && ' · Legacy access — remove before saving'}</span></span></label>;
