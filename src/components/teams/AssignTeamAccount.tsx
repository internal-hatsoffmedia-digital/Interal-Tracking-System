...
export default function AssignTeamAccount({ teams, onSaved }: { teams: Team[]; onSaved: () => Promise<void> }) {
 ...
 <fieldset disabled={busy||loading} className="grid grid-cols-1 items-end gap-3 md:grid-cols-[minmax(0,2fr)_minmax(0,1fr)_auto]">
 ...
 <label className="grid min-w-0 gap-1 text-sm">Team<select required className="min-w-0 w-full rounded-lg border p-2" value={team} onChange={e=>setTeam(e.target.value)}><option value="">Select team</option>{teams.filter(t=>t.is_active).map(t=><option key={t.id} value={t.id}>{t.name}</option>)}</select></label>
