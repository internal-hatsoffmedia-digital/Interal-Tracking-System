    ...
    <div className="min-w-0 space-y-6">
      ...
      {isFormOpen && !editingEmployee && currentProfile?.role === "admin" && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4 backdrop-blur-xs">
          <section role="dialog" aria-modal="true" aria-label="Add employee from Authentication" className="flex max-h-[92vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
            <div className="flex shrink-0 items-center justify-between border-b border-slate-200 px-6 py-4">
              <span className="text-sm font-semibold text-slate-900">Provision Employee</span>
              <button type="button" autoFocus onClick={handleCloseForm} className="rounded-xl px-3 py-1.5 text-xs font-semibold text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition border border-slate-200" aria-label="Close modal">
                Close
              </button>
            </div>
            <div className="overflow-y-auto p-6">
              <AuthEmployeeForm onSaved={async () => { await loadEmployees(); handleCloseForm(); }} />
            </div>
          </section>
        </div>
      )}
