...
try{
 ...
 await context.clearCookies();await page.evaluate(()=>localStorage.clear());await page.goto(origin+'/login');await page.getByRole('button',{name:'Sign In',exact:true}).waitFor();await page.screenshot({path:'test-artifacts/brand-login-mobile.png',fullPage:true});await page.setViewportSize({width:1440,height:1000});await page.screenshot({path:'test-artifacts/brand-login-desktop.png',fullPage:true});assert.equal(await page.getByRole('button',{name:'Sign In',exact:true}).evaluate(el=>getComputedStyle(el).backgroundColor),'rgb(255, 204, 0)');assert.deepEqual(errors,[]);console.log('PASS future dashboard views, theme, mobile navigation and overflow');await context.close();
